...
class _InvasiveSectionPageState extends State<InvasiveSectionPage>
  ...
  late int _tabBarCount = 2;

  Future<void> _loadData() async {
    setState(() => isRunning = true);
    try {
      final visual = await sectionRepository.getVisualSection(widget.sectionId);
      final invasive = await sectionRepository.getInvasiveSection(
        widget.sectionId,
      );
      final conclusive = await sectionRepository.getConclusiveSection(
        widget.sectionId,
      );
      ...
      if (visual == null) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Error loading section data.')),
          );
        }
      ...
      currentInvasiveSection.id ??= CouchbaseDocument.generateId();
      ...
      currentConclusiveSection.id ??= CouchbaseDocument.generateId();

      _tabBarCount = currentInvasiveSection.postinvasiverepairsrequired ? 3 : 2;
      setInitialValues();
    } catch (e) {
      debugPrint('Error loading invasive section data: $e');
    }
    if (mounted) setState(() => isRunning = false);
  }
  ...
  void initState() {
    super.initState();
    sectionRepository = context.read<SectionRepository>();
    imageRepository = context.read<ImageRepository>();
    isNewSection = widget.isNewSection;
    capturedImages.clear();
    capturedInvasiveImages.clear();
    capturedConclusiveImages.clear();
  ...
  void setInitialValues() {
    ...
    final eeeC = currentConclusiveSection.eeeconclusive;
    if (eeeC != null && eeeC.isNotEmpty) {
      _eeeConclusive = ExpectancyYears.values.firstWhere(
        (e) => e.name == eeeC,
        orElse: () => ExpectancyYears.values.first,
      );
    }
    final lbcC = currentConclusiveSection.lbcconclusive;
    if (lbcC != null && lbcC.isNotEmpty) {
    ...
    final aweC = currentConclusiveSection.aweconclusive;
    if (aweC != null && aweC.isNotEmpty) {
  ...
  save(BuildContext context) async {
    ...
    final saveResult = await sectionRepository.addupdateInvasiveSection(
      currentInvasiveSection,
      _invasiveDescriptionController.text,
      postInvasiveRepairsRequired,
    );
    ...
    if (postInvasiveRepairsRequired) {
      if (!invasiveRepairsCompleted) {
        conclusiveSaveResult = await sectionRepository
  ...
  void removePhoto(BuildContext context, int index, bool isConclusive) {
    if (isConclusive) {
      sectionRepository.removeConclusiveImageUrl(
        currentConclusiveSection,
        capturedConclusiveImages[index],
      );
    } else {
      sectionRepository.removeInvasiveImageUrl(
        currentInvasiveSection,
  ...
  gotoImageEditorPage(
    ...
    if (editedImage != null) {
      ...
      if (isConclusive) {
        await sectionRepository.removeConclusiveImageUrl(
          currentConclusiveSection,
          capturedImage,
        );
        setState(() {
        ...
        await sectionRepository.removeInvasiveImageUrl(
          currentInvasiveSection,
          capturedImage,
        );
        setState(() {
  ...
  visualSectionWidget() {
    return SingleChildScrollView(child: sectionForm(context));
  }
